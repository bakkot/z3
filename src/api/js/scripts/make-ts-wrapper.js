...
export async function init(initModule: any) {
  ...
  const outAddress = Mod._malloc(${BYTES_TO_ALLOCATE_FOR_OUT_PARAMS});
  const outUintArray = (new Uint32Array(Mod.HEAPU32.buffer, outAddress, 4));
  const getOutUint = (i: ${getValidOutArrayIndexes(4)}) => outUintArray[i];
  const outIntArray = (new Int32Array(Mod.HEAPU32.buffer, outAddress, 4));
  const getOutInt = (i: ${getValidOutArrayIndexes(4)}) => outIntArray[i];
  const outUint64Array = (new BigUint64Array(Mod.HEAPU32.buffer, outAddress, 2));
  const getOutUint64 = (i: ${getValidOutArrayIndexes(8)}) => outUint64Array[i];
  const outInt64Array = (new BigInt64Array(Mod.HEAPU32.buffer, outAddress, 2));
  const getOutInt64 = (i: ${getValidOutArrayIndexes(8)}) => outInt64Array[i];
